...
export const HeroContent = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Create dynamic stars on mouse move
      if (Math.random() > 0.9) {
        createStar(x, y);
      }
    };
    ...
    const createStar = (x: number, y: number) => {
      ...
      profileRef.current.appendChild(star);
    ...
    const createMeteor = () => {
      ...
      profileRef.current.appendChild(meteor);
    ...
    profileRef.current?.addEventListener('mouseleave', resetTransform);
    
    return () => {
      document.removeEventListener('mousemove', handleMouseMove);
      profileRef.current?.removeEventListener('mouseleave', resetTransform);
    ...
    >
      ...
      >
        <div className="relative w-[300px] h-[300px] profile-container group">
          ...
          {/* Orbiting particles */}
          <div className="absolute w-6 h-6 rounded-full bg-purple-500 opacity-70 orbiting" style={{ transformOrigin: 'center center', left: 'calc(50% - 3px)', top: 'calc(50% - 3px)' }}></div>
          <div className="absolute w-4 h-4 rounded-full bg-cyan-500 opacity-70 orbiting" style={{ transformOrigin: 'center center', left: 'calc(50% - 2px)', top: 'calc(50% - 2px)', animationDelay: '-2s' }}></div>
          <div className="absolute w-5 h-5 rounded-full bg-pink-400 opacity-70 orbiting" style={{ transformOrigin: 'center center', left: 'calc(50% - 2.5px)', top: 'calc(50% - 2.5px)', animationDelay: '-4s' }}></div>
          
          {/* Main profile picture */}
          <div className="absolute inset-0 rounded-full overflow-hidden border-4 border-[#030014] z-10 hover-float">
            <Image
              src="/profile pic.jpg"
              alt="Aditi Singh"
              layout="fill"
          ...
          {/* Enhanced pulsating border */}
          <div className="absolute inset-[-5px] rounded-full pulsating-border"></div>
          ...
          <div className="absolute -top-4 -right-4 w-10 h-10 bg-purple-500 rounded-full opacity-80 hover-float"></div>
          <div className="absolute -bottom-4 -left-4 w-6 h-6 bg-cyan-500 rounded-full opacity-80 hover-float" style={{ animationDelay: '-1s' }}></div>
          <div className="absolute top-1/2 -right-8 w-8 h-8 bg-violet-500 rounded-full opacity-80 hover-float cosmic-glow" style={{ animationDelay: '-2s' }}></div>
          <div className="absolute -top-8 left-1/4 w-5 h-5 bg-pink-400 rounded-full opacity-70 hover-float" style={{ animationDelay: '-1.5s' }}></div>
          <div className="absolute -bottom-6 right-1/4 w-7 h-7 bg-blue-400 rounded-full opacity-70 hover-float" style={{ animationDelay: '-0.5s' }}></div>
